    ...
    const stats = {
      ...
      assigneeBreakdown: tasks.reduce((acc, t) => {
        if (!acc[t.assigned_to]) {
          acc[t.assigned_to] = { total: 0, completed: 0, inProgress: 0, pending: 0, blocked: 0 }
        }
        acc[t.assigned_to].total++
        if (t.status === 'Completed') acc[t.assigned_to].completed++
        if (t.status === 'In Progress') acc[t.assigned_to].inProgress++
        if (t.status === 'Pending') acc[t.assigned_to].pending++
        if (t.status === 'Blocked') acc[t.assigned_to].blocked++
        return acc
