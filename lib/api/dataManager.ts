...
export class DataValidator {
  ...
  static validateTask(task: Partial<Task>): string[] {
    ...
    if (!task.vehicle_id?.trim()) errors.push('Vehicle ID is required')
    if (!task.assigned_to?.trim()) errors.push('Assignee is required')
  ...
  static validateLocation(location: Partial<Location>): string[] {
    ...
    if (!location.duration?.trim()) errors.push('Duration is required')
